...
public class ComportementSupraIntelligent2 implements ComportementAbstrait{
    ...
    {	
    	...
    	if(asc.getListePersonne().size()==0){
		    ...
		    //Si la personne veut monter et que l'ascenseur monte
                    if(Immeuble.ascenseurLePlusProche(asc).getListePersonne().get(0).veutMonter() == true && asc.isMonte()){
                        attendre();
                    }
		    //Si la personne veut descendre et que l'ascenseur descend
                    else if(Immeuble.ascenseurLePlusProche(asc).getListePersonne().get(0).veutMonter() == false && !asc.isMonte()){
                        attendre();
                    }
		    //Si la personne veut monter et que l'ascenseur descend => on change le sens de marche
    ...
    //Si il y a qq1 dans l'ascenseur
            ...
            if(etageDestPers0<asc.getEtageCourant()){
                asc.descendre();
                if(asc.quelqunVeutDessendreDeAsc()!=null){//Si la personne veut descendre et que l'ascenseur descend
                        attendre();
                }
            }
